...
if argv[1][0:2] == '-y': # Yubico Yubikey
    if (argv[1][2] == 'd') and (len(argv)>2):
        ...
        cur.execute("SELECT COUNT(*) FROM yubikeys WHERE nickname = '" + nickname + "'")
        rows = cur.fetchone()[0]
        cur.execute("SELECT * FROM yubikeys WHERE nickname = '" + nickname + "'")
        if rows == 0:
            print 'Key not found.'
        else:
            cur.execute("SELECT COUNT(*) FROM yubikeys WHERE nickname = '" + nickname + "' AND active = '1'")
            rows = cur.fetchone()[0]
        ...
        if rows == 0:
            ...
            cur.execute("SELECT COUNT(*) FROM yubikeys WHERE nickname = '" + nickname + "' AND active = '0'")
            rows = cur.fetchone()[0]
        ...
        if ((len(argv[2])<=16) and (len(argv[3]) <= 16) and (len(argv[4]) <= 12) and (len(argv[5])<=32)):
            cur.execute("SELECT COUNT(*) FROM yubikeys WHERE nickname = '" + argv[2] + "' OR publicname = '" + argv[3] + "'")
            rows = cur.fetchone()[0]
        ...
        if rows != 0:
            print " " + str(rows) + " keys into database:"
    ...
    if (argv[1][2] == 'd') and (len(argv)>2):
        nickname = re.escape(argv[2])
        cur.execute("SELECT COUNT(*) FROM oathtokens WHERE nickname = '" + nickname + "'")
        rows = cur.fetchone()[0]
        cur.execute("SELECT * FROM oathtokens WHERE nickname = '" + nickname + "'")
        if rows == 0:
            print 'Key not found.'
        else:
            cur.execute("SELECT COUNT(*) FROM oathtokens WHERE nickname = '" + nickname + "' AND active = '1'")
            rows = cur.fetchone()[0]
        ...
        if rows == 0:
            ...
            cur.execute("SELECT COUNT(*) FROM oathtokens WHERE nickname = '" + nickname + "' AND active = '0'")
            rows = cur.fetchone()[0]
        ...
        if (len(argv[2])<=16) and (len(argv[3]) <= 16) and (len(argv[4]) <= 40):
            cur.execute("SELECT COUNT(*) FROM oathtokens WHERE nickname = '" + argv[2] + "' OR publicname = '" + argv[3] + "'")
            rows = cur.fetchone()[0]
...
#MARK
    ...
    if (argv[1][2] == 'a') and (len(argv)>2):
        nickname = re.escape(argv[2])
        cur.execute("SELECT COUNT(*) FROM apikeys WHERE nickname = '" + nickname + "'")
        rows = cur.fetchone()[0]
        ...
        if rows != 0:
            ...
            quit()
        cur.execute("SELECT COUNT(id) FROM apikeys ORDER BY id DESC LIMIT 1")
        rows = cur.fetchone()[0]
